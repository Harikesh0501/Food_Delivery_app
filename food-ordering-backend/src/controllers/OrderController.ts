...
const stripeWebhookHandler = async (req: Request, res: Response) => {
  ...
  if (event.type === "checkout.session.completed") {
    ...
    if (!orderId || !mongoose.Types.ObjectId.isValid(orderId)) {
      console.error(
        `Invalid orderId in Stripe webhook: ${orderId}.`
      );
...
const createCheckoutSession = async (req: Request, res: Response) => {
  try {
    ...
    // ===== DEMO MODE: Skip Stripe if API key is not configured =====
    if (!STRIPE) {
      console.log("⚠️  Stripe not configured — using DEMO checkout mode");
    ...
    console.log(error);
    res.status(500).json({ message: error?.raw?.message || error?.message || "Checkout failed" });
  }
};
