...
type FormData = { email: string; password: string; name: string; role: string };
...
const RegisterPage = () => {
  ...
  const onSubmit = async (data: FormData) => {
    setIsLoading(true);
    try {
      const res = await axiosInstance.post("/api/auth/register", { ...data, role: selectedRole });
      if (res.data?.token) {
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Select value={selectedRole} onValueChange={(val) => setSelectedRole(val)}>
              ...
              <SelectContent>
                <SelectItem value="user">Order Food (Customer)</SelectItem>
                <SelectItem value="restaurant_owner">Manage a Restaurant (Owner)</SelectItem>
              </SelectContent>
            </Select>
          </div>
